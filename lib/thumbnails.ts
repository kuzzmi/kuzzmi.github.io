import { createCanvas, SKRSContext2D, GlobalFonts } from "@napi-rs/canvas"; // For canvas.
import path from "path"; // For creating files for our images.
...
GlobalFonts.registerFromPath("fonts/PTSans-Italic.ttf", "PTSans-Italic");
GlobalFonts.registerFromPath("fonts/PTSans-Bold.ttf", "PTSans");
GlobalFonts.registerFromPath("fonts/AppleColorEmoji.ttf", "AppleEmoji");

type LinePlacement = [string, number, number];
...
// imagePath: this is where the image will be saved
...
export const generateMainImage = async function (
  ...
  gradientColors.forEach((g, idx) => {
    grd.addColorStop(idx, g);
  });
  ctx.fillStyle = grd;
  // Fill our gradient
  ...
  ctx.font = "50px PTSans-Italic";
  ctx.fillStyle = "rgba(255,255,255,0.8)";
  ctx.fillText(articleCategory, 85, 553 - wrappedText[1] - 120); // 853 - 200 for emoji, -100 for line height of 1

  // if (fs.existsSync(`${imagePath}/${canonicalName}.png`)) {
  ...
  try {
    ...
    // Save file
    fs.writeFileSync(
      path.join(`${imagePath}/${canonicalName}.png`),
      canvasData
    );
  } catch (e) {
    console.log(e);
  ...
  //     path.join(__dirname, "../", `${imagePath}/${canonicalName}.png`)
  //   );
  //   encoder.quality(30);
  //   await encoder.write(
  //     `${imagePath}/${canonicalName}.webp`,
